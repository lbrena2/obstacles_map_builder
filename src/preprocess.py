...
def preprocess(path, extractors, tolerance='1s',
               ...
               coords=[(0, 0)], interval='15s', delta=0.1, fillna=None):
    ...
    for index, file in enumerate(files):
        ...
        if input_cols is None:
            input_cols = [
                col for col in df.columns.values if col not in target_cols]
        ...
        for col in input_cols + output_cols:
            shape = np.array(df[col].iloc[0]).shape
            store = h5f.create_dataset('bag' + str(index) + '/' + col,
                                       shape=(length,) + shape, maxshape=(None,) + shape,
...
if __name__ == '__main__':
    ...
    path = '/home/usi/catkin_ws/src/obstacles_map_builder/data/bag/'
    ...
    extractors = {
        ...
        # prefix + '/proximity/laser': lambda m: {
        #     'target_sensors': np.array(m.intensities)
        # },
    ...
    res = preprocess(path=path, extractors=extractors,
                     input_cols=input_cols, target_cols=target_cols,
                     coords=coords, tolerance='2s', fillna=-1.0)
